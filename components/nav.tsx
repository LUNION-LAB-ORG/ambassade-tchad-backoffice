      ...
      <nav className=" space-y-1 px-5">
        {links.map(({ icon, title, active }, index) => (
          <Button
            key={`link-${index}`}
            fullWidth
            className="capitalize justify-start hover:bg-transparent hover:text-default-600"
            variant="ghost"
          >
            <span
              className={cn(
                "h-2 w-2 me-2 rounded-full  transition-all duration-150 ring-0   bg-default ring-default ring-opacity-30",
                {
                  ...
                  "bg-destructive ring-destructive":
                    title === "team" || title === "promotions",
                  "bg-success ring-success":
                    title === "low" || title === "social",
                  "bg-warning ring-warning": title === "medium",
                  "bg-primary ring-primary":
                    title === "high" || title === "buisness",
    ...
    <nav className=" space-y-1 px-5">
      {links.map(({ icon, title, active }, index) => (
        <Button
          ...
          color="secondary"
          fullWidth
          className="hover:ring-0 hover:ring-transparent justify-start"
