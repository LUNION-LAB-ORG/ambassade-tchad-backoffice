    ...
    return (
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <button type="button" className="relative hidden focus:ring-none focus:outline-none md:h-8 md:w-8 md:bg-embassy-blue-100 hover:bg-embassy-blue-200 text-embassy-blue-700 rounded-full md:flex flex-col items-center justify-center transition-all duration-300 dark:md:bg-embassy-blue-800/30 dark:text-embassy-blue-300 dark:hover:bg-embassy-blue-700/40">
                ...
                <DropdownMenuLabel>
                    <div className="flex justify-between px-4 py-3 border-b border-embassy-blue-100 dark:border-embassy-blue-700/30">
                        ...
                        <div className="text-embassy-blue-600 text-xs md:text-right dark:text-embassy-blue-400">
                            <Link href="/notifications" className="underline hover:text-embassy-blue-800 transition-colors dark:hover:text-embassy-blue-300">
                                Voir tout
                            </Link>
                        </div>
                    </div>
                ...
                <div className="h-[300px] xl:h-[350px]">
                    <ScrollArea className="h-full">
                        {notifications.map((item: Notification, index: number) => (
                            <DropdownMenuItem
                                key={`inbox-${index}`}
                                className="flex gap-9 py-3 px-4 cursor-pointer group hover:bg-embassy-blue-50 transition-colors duration-200 dark:hover:bg-embassy-blue-800/30"
                            >
                                <div className="flex items-start gap-3 flex-1">
                                    ...
                                    <div className="flex-1 flex flex-col gap-1">
                                        <div className="text-sm text-embassy-blue-800 group-hover:text-embassy-blue-900 font-medium truncate dark:text-embassy-blue-200 dark:group-hover:text-embassy-blue-100">
                                            {item.title}
                                        </div>
                                        <div className="text-xs text-embassy-blue-600 group-hover:text-embassy-blue-700 font-normal line-clamp-1 dark:text-embassy-blue-400 dark:group-hover:text-embassy-blue-300">
                                            {item.desc}
                                        </div>
                                        <div className="text-embassy-blue-500 group-hover:text-embassy-blue-600 text-xs dark:text-embassy-blue-500 dark:group-hover:text-embassy-blue-400">{item.date}</div>
                                    </div>
                                </div>
                                {item.unreadmessage && (
